...
Section "install"
    ...
    # Registry entries for Add/Remove Programs (user-level)
    ...
    WriteRegStr HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\${COMPANYNAME} ${APPNAME}" "DisplayIcon" "$\"$INSTDIR\syswatch-agent-windows.exe$\""
