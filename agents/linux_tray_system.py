...
class SysWatchLinuxTray:
    def __init__(self):
        ...
        user_config_dir = Path.home() / ".config" / "SysWatch"
        system_config_dir = Path("/opt/syswatch")
        
        if user_config_dir.exists():
            # User installation
            user_config_dir.mkdir(parents=True, exist_ok=True)
            self.config_file = user_config_dir / "tray_config.json"
            self.install_type = "user"
        elif system_config_dir.exists():
            # System installation - store user config in home
            user_config_dir.mkdir(parents=True, exist_ok=True)
            self.config_file = user_config_dir / "tray_config.json"
    ...
    def create_simple_gui(self):
        ...
        tk.Label(root, text=f"Type: {self.install_type} service", font=('Arial', 8)).pack(pady=5)
    ...
    def init_cli(self):
        ...
        while True:
            ...
            print(f"Installation type: {self.install_type} service")
            print("\nOptions:")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if self.install_type == "system":
                    ...
                    subprocess.run(['sudo', 'cp', '/tmp/syswatch-update', str(install_dir / 'syswatch-agent-linux')], capture_output=True)
    ...
    def show_about(self):
        ...
        install_path = '/opt/syswatch/' if self.install_type == 'system' else '~/.local/share/SysWatch/'
...
Service: syswatch-agent ({self.install_type} service)
